...
class KiroProvider(BaseProvider):
    ...
    @staticmethod
    def _parse_aws_headers(data: bytes) -> dict:
        """解析 AWS event-stream 二进制 headers"""
        headers = {}
        offset = 0
    ...
    def parse_response(self, raw: bytes) -> Dict[str, Any]:
        ...
        while pos < len(raw):
            ...
            # CRC32 校验 - 借鉴 kiro.rs 的实现
            # Prelude CRC: 前 8 字节的 CRC32
            prelude_crc = int.from_bytes(raw[pos+8:pos+12], 'big')
            expected_prelude_crc = binascii.crc32(raw[pos:pos+8]) & 0xFFFFFFFF
            if prelude_crc != expected_prelude_crc:
                pos += total_len  # CRC 不匹配，跳过此消息
                continue
            
            # Message CRC: 整个消息（不含最后 4 字节）的 CRC32
            msg_crc = int.from_bytes(raw[pos+total_len-4:pos+total_len], 'big')
            expected_msg_crc = binascii.crc32(raw[pos:pos+total_len-4]) & 0xFFFFFFFF
            ...
            try:
                parsed_hdrs = self._parse_aws_headers(headers_data)
            ...
            if payload_start < payload_end:
                try:
                            ...
                            if isinstance(c, str):
                                ...
                                for item in c:
                                    if isinstance(item, dict) and 'text' in item:
                                        result["content"].append(item['text'])
                                    elif isinstance(item, str):
                                        result["content"].append(item)
                            elif c is not None:
                                result["content"].append(str(c))
                    ...
                    # 处理 contextUsageEvent — 借鉴 kiro.rs
                    if event_type == 'contextUsageEvent' or 'contextUsagePercentage' in payload:
                        pct = payload.get('contextUsagePercentage', 0)
                        result["context_usage_percentage"] = pct
                        # 根据百分比计算实际 input_tokens (context_window = 200k)
                        result["input_tokens"] = int(pct * 200000 / 100)
                        if pct >= 100.0:
                            result["stop_reason"] = "model_context_window_exceeded"
                    
                    # 处理 exceptionEvent — 借鉴 kiro.rs
                    # kiro.rs: message_type='exception' -> parse :exception-type header
                    if msg_type == 'exception':
                        exc_type = parsed_hdrs.get(':exception-type', '')
                        if exc_type == 'ContentLengthExceededException':
                            result["stop_reason"] = "max_tokens"
                    elif event_type == 'exceptionEvent' or 'exceptionType' in payload:
                        exc_type = payload.get('exceptionType', '')
                        if exc_type == 'ContentLengthExceededException':
                            result["stop_reason"] = "max_tokens"
        ...
        for tool_id, tool_data in tool_input_buffer.items():
            ...
            try:
                ...
                # 不使用 {"raw": ...} 避免污染工具参数名（这是之前 Write 工具失败的根因）
                print(f"[KiroProvider] Failed to parse tool input JSON for {tool_data['name']}: {type(e).__name__}: {str(e)[:100]}")
                print(f"[KiroProvider] Raw input ({len(input_str)} chars): {input_str[:500]}")
                input_json = {}
