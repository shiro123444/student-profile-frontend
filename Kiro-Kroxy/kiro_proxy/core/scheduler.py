        ...
        for acc in state.accounts:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # 401 先尝试刷新 token（参考 kiro.rs: 401 视为凭据问题，先刷新再判断）
                    print(f"[HealthCheck] 账号认证失败，尝试刷新 token: {acc.name}")
                    refresh_ok, refresh_msg = await acc.refresh_token()
                    if refresh_ok:
                        print(f"[HealthCheck] Token 刷新成功: {acc.name}")
                    else:
                        acc.status = CredentialStatus.UNHEALTHY
                        print(f"[HealthCheck] Token 刷新失败，标记不健康: {acc.name} - {refresh_msg}")
