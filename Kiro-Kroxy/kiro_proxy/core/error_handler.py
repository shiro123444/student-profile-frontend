...
def classify_error(status_code: int, error_text: str) -> KiroError:
    ...
    # 7. 请求格式错误检测（Improperly formed request - 通常是截断后历史不合法）
    if status_code == 400 and ("improperly formed" in error_lower or "malformed" in error_lower):
        return KiroError(
            type=ErrorType.CONTENT_TOO_LONG,
            status_code=status_code,
            message=error_text,
            user_message="请求格式错误，正在自动修复并重试",
            should_retry=True,
        )
