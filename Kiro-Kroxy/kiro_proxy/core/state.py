...
class ProxyState:
    ...
    def _filter_by_model(self, accounts: list, model: Optional[str] = None) -> list:
        """按模型过滤账号（Enterprise 模型只用 Enterprise 账号）"""
        if not model:
            return accounts
        if not accounts:
            return accounts  # 输入为空时直接返回，避免误报警告
        from ..config import is_enterprise_only_model
        if not is_enterprise_only_model(model):
            return accounts
        # opus 等模型只用 Enterprise 账号
        enterprise_accounts = []
        for acc in accounts:
            creds = acc.get_credentials()
            if creds and creds.provider == "Enterprise":
                enterprise_accounts.append(acc)
        if enterprise_accounts:
            return enterprise_accounts
        # 没有 Enterprise 账号可用时，回退使用所有可用账号（让 Kiro API 自行判断权限）
        print(f"[State] 警告: 模型 {model} 推荐使用 Enterprise 账号，当前无可用 Enterprise 账号，尝试使用现有账号")
        return accounts

    def get_available_account(self, session_id: Optional[str] = None, model: Optional[str] = None) -> Optional[Account]:
        """获取可用账号（支持会话粘性和模型过滤）"""
            ...
            if time.time() - ts < 60:
                for acc in self.accounts:
                    if acc.id == account_id and acc.is_available():
                        # 检查会话粘性账号是否支持请求的模型
                        from ..config import is_enterprise_only_model
                        if model and is_enterprise_only_model(model):
                            creds = acc.get_credentials()
                            if not (creds and creds.provider == "Enterprise"):
                                break  # 跳出粘性，重新选择
                        self.session_timestamps[session_id] = time.time()
                        return acc
        
        available = [a for a in self.accounts if a.is_available()]
        available = self._filter_by_model(available, model)
        if not available:
            # 自动恢复：当所有账号都不可用时，重置 COOLDOWN 和 UNHEALTHY 状态的账号
            # 参考 kiro.rs 自愈机制：所有凭据均已被自动禁用时，重置失败计数并重新启用（等价于重启）
            recoverable_accounts = [
                a for a in self.accounts 
                if a.enabled and a.status in (CredentialStatus.COOLDOWN, CredentialStatus.UNHEALTHY)
            ]
            if recoverable_accounts:
                print(f"[State] 所有账号不可用，自动恢复 {len(recoverable_accounts)} 个账号 (COOLDOWN/UNHEALTHY -> ACTIVE)")
                for acc in recoverable_accounts:
                    acc.status = CredentialStatus.ACTIVE
                    acc.error_count = 0  # 重置错误计数（kiro.rs: failure_count = 0）
                    quota_manager.restore(acc.id)
                available = [a for a in self.accounts if a.is_available()]
                available = self._filter_by_model(available, model)
            if not available:
                return None
    ...
    def get_next_available_account(self, exclude_id: str, model: Optional[str] = None) -> Optional[Account]:
        """获取下一个可用账号（排除指定账号，支持模型过滤）"""
