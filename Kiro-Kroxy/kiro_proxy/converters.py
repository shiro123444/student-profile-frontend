...
def fix_history_alternation(history: List[dict], model_id: str = "claude-sonnet-4") -> List[dict]:
    ...
    for i, item in enumerate(history):
        ...
        if is_user:
            # 如果是第一条消息且有 toolResults，清除它（没有前置 assistant 的 toolUses 与之配对）
            if not fixed:
...
def convert_kiro_response_to_anthropic(result: dict, model: str, msg_id: str) -> dict:
    ...
    # 使用从 contextUsageEvent 计算的真实 token 数
    input_tokens = result.get("input_tokens", 0) or 100
    output_tokens = result.get("output_tokens", 0) or 100
    
    # 如果上下文使用率 >= 100%，设置 stop_reason 让 Claude Code 自动 compact
    stop_reason = result["stop_reason"]
    pct = result.get("context_usage_percentage")
