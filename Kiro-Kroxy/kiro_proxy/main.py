...
from .config import (
    MODELS_URL, THINKING_MODEL_VARIANTS, BUILTIN_KIRO_MODELS,
...
async def models():
    ...
    if all_models:
        ...
        for mid, mname in [("claude-opus-4.5", "Claude Opus 4.5"), ("claude-opus-4.6", "Claude Opus 4.6")]:
            if mid not in all_models:
                model_list.append({
        ...
        for model_id, model_info in get_custom_models().items():
            if model_id not in existing_ids:
                model_list.append({
                    ...
                    "context_window": MODEL_CONTEXT_WINDOWS.get(model_id, 200000),
                    "max_tokens": MODEL_MAX_OUTPUT_TOKENS,
                })
        return {
            "object": "list",
